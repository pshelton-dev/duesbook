...
import { getMemberDetail, listMembers } from '../../../src/data/members'
import { useQuery } from '../../../src/ui/books'
import { Button, Card, ListRow, PinnedButton, Screen, SectionTitle, StatusDot } from '../../../src/ui/components'
...
export default function MemberDetail(): React.JSX.Element {
  ...
  return (
    <>
      ...
      {owes && current && (
        <PinnedButton
          title={`Record payment · ${formatCents(row!.owedCents)}`}
          onPress={() => router.push({ pathname: '/payment', params: { periodId: String(current.id), memberId: String(memberId) } })}
        />
      )}
    </>
  )
