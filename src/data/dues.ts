...
export function listPeriods(db: Db): DuesPeriodRow[] {
...
export function createPeriod(db: Db, input: DuesPeriodInput): number {
...
export function updatePeriod(db: Db, id: number, input: DuesPeriodInput): void {
  validatePeriod(input)
  const result = db
    .prepare(
...
function latestPeriod(db: Db): PeriodRow | undefined {
...
export function suggestNextPeriod(db: Db): DuesPeriodInput | null {
 ...
 */
export function ensurePeriodsCurrent(db: Db): number {
...
export function getRoster(db: Db, periodId: number): DuesRoster {
  const period = db
...
export function listUnallocated(db: Db): UnallocatedDeposit[] {
  const rows = db
    .prepare(
...
export function recordPayment(db: Db, p: RecordDuesPayment): void {
