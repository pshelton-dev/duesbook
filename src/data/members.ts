...
export function currentPeriod(db: Db): PeriodRow | null {
 ...
 */
export function computeDuesTotals(db: Db): Map<number, DuesTotals> {
  const rows = db
    .prepare(
...
export function createMember(db: Db, m: MemberInput): void {
  validate(m)
...
export function updateMember(db: Db, id: number, m: MemberInput): void {
  validate(m)
  const result = db
    .prepare(
...
export function importMembers(db: Db, members: WizardMember[]): number {
  const now = new Date().toISOString()
...
export function getMemberDetail(db: Db, id: number): MemberDetail {
